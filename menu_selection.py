...
def sauvegarde_caracteristique(selection_niveau, options):
    with open("save/caracteristique_dernier_donjon.txt", "w") as fichier_sauvegarde:
        if options[0]:
            fichier_sauvegarde.write("A\n")
        else:
            fichier_sauvegarde.write("M\n")
        fichier_sauvegarde.write(selection_niveau + "\n")
        if options[1]:
...
def main(debug=False):
    ...
    if selection_niveau[0] in ["n", "t"]:
        niveau_final = int(selection_niveau[-1])
    elif selection_niveau[0] == "l":
        niveau_final = 0
    else:
        niveau_final = - (recuperer_indice(niveau_custom, selection_niveau) + 1)

    sauvegarde_caracteristique(selection_niveau, option)

    if option[0]:
        if option[1]:
            wall_is_you_graphic.main(niveau_final, True)
        else:
            wall_is_you_graphic.main(niveau_final)
    else:
        if option[1]:
            wall_is_you_graphic_automatique.main(niveau_final, True)
        else:
            wall_is_you_graphic_automatique.main(niveau_final)
