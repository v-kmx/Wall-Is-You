...
Nous avons également implémenté le déplacement des dragons. Ces derniers se déplaçeront aléatoirement mais uniquement à la fin du tour de l'aventurier (dans notre code, il s'agit du moment où l'intention de l'aventurier vaut 1 et que l'aventurier est entrain de terminer son tour. Au départ, lorsque cette fonction a été implémentée, il y avait des bugs du genre : Les dragons qui ne se déplacent pas du tôt, Les dragons se déplacent à chaque mouvement de l'aventurier et les dragons se mettaient à traverser les murs (détection des voisins incorrecte..). Les problèmes précédemment mentionnés ont été résolus (pour le problèmes des dragons qui passaient à travers les murs, on a décidé de reprendre la logique de verifier_validite_intention de l'aventurier pour que les dragons respectent, tout comme l'aventurier, les murs et les portes.

Nous avons également commencer à penser aux futur éléments à ajouter, ce qui inclut:
le pathfinding
refonte des menues
